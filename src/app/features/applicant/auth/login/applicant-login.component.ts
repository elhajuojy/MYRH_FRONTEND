...
export class ApplicantLoginComponent {
  ...
  onLoginSubmit(form:FormGroup):void{
    //: applicant can take login by email and password to Marocannonces site for displaying job offers
    //RANDOM NUMBER PLEASE
    console.log(
      this.randomIntFromInterval(100000,999999)
    )

    this.applicantService.authenticateApplicant(form.value.email,form.value.password).
    subscribe((data:any)=>{
      console.log(data)
